...
const DEFAULT_GROUPS = [
  { id: "g1", name: "Large Venues" },
  { id: "g2", name: "Meeting Rooms" },
  { id: "g3", name: "Classrooms" },
  { id: "g4", name: "Outdoor Locations" },
  { id: "g5", name: "ICT Resources" },
];

const RESOURCE_SEED = [
  { id: "r1", name: "Hall Court 1", groupId: "g1", slots: 1, capacity: 200, requiresApproval: true },
  { id: "r2", name: "Hall Court 2", groupId: "g1", slots: 1, capacity: 150, requiresApproval: true },
  { id: "r3", name: "Hall Hermitage", groupId: "g1", slots: 1, capacity: 60, requiresApproval: false },
  { id: "r4", name: "Theatre", groupId: "g1", slots: 1, capacity: 300, requiresApproval: true },
  { id: "r5", name: "LaValla", groupId: "g1", slots: 1, capacity: 40, requiresApproval: false },
  { id: "r6", name: "Chapel", groupId: "g1", slots: 1, capacity: 120, requiresApproval: true },
  { id: "r7", name: "Meeting Room A", groupId: "g2", slots: 2, capacity: 10, requiresApproval: false },
  { id: "r8", name: "Meeting Room B", groupId: "g2", slots: 1, capacity: 6, requiresApproval: false },
  { id: "r9", name: "Room 101", groupId: "g3", slots: 1, capacity: 30, requiresApproval: false },
  { id: "r10", name: "Room 204", groupId: "g3", slots: 1, capacity: 28, requiresApproval: false },
  { id: "r11", name: "Sports Oval", groupId: "g4", slots: 1, capacity: 500, requiresApproval: true },
  { id: "r12", name: "Laptop Trolley", groupId: "g5", slots: 3, capacity: null, requiresApproval: false },
];
...
const DEFAULT_PERIODS = [
  { id: "morning", label: "Morning", type: "break", start: "08:00", end: "08:20" },
  { id: "homeroom", label: "Homeroom", type: "period", start: "08:20", end: "08:35" },
  { id: "p1", label: "Period 1", type: "period", start: "08:40", end: "09:30" },
  { id: "p2", label: "Period 2", type: "period", start: "09:35", end: "10:25" },
  { id: "recess", label: "Recess", type: "break", start: "10:25", end: "10:45" },
  { id: "p3", label: "Period 3", type: "period", start: "10:45", end: "11:35" },
  { id: "p4", label: "Period 4", type: "period", start: "11:40", end: "12:30" },
  { id: "lunch", label: "Lunch", type: "break", start: "12:30", end: "13:15" },
  { id: "p5", label: "Period 5", type: "period", start: "13:15", end: "14:05" },
  { id: "p6", label: "Period 6", type: "period", start: "14:10", end: "15:00" },
];
const HOURLY_PERIODS = [
  { id: "h9", label: "9–10am", type: "period", start: "09:00", end: "10:00" },
  { id: "h10", label: "10–11am", type: "period", start: "10:00", end: "11:00" },
  { id: "h11", label: "11am–12pm", type: "period", start: "11:00", end: "12:00" },
  { id: "hlunch", label: "Lunch", type: "break", start: "12:00", end: "13:00" },
  { id: "h13", label: "1–2pm", type: "period", start: "13:00", end: "14:00" },
  { id: "h14", label: "2–3pm", type: "period", start: "14:00", end: "15:00" },
  { id: "h15", label: "3–4pm", type: "period", start: "15:00", end: "16:00" },
];
function clonePeriods(list) { return list.map((p) => ({ ...p, id: uid() })); }
function buildDefaultPeriodsByGroup(groupList) {
  const map = {};
  for (const g of groupList) {
    map[g.id] = clonePeriods(g.id === "g2" ? HOURLY_PERIODS : DEFAULT_PERIODS);
  }
  return map;
}
...
const USER_SEED = [
  { id: "u1", name: "A. Marsh", role: "admin" },
  { id: "u2", name: "Julie Stewart", role: "user" },
  { id: "u3", name: "Barry Graham", role: "user" },
  { id: "u4", name: "Trudy Sawyer", role: "user" },
];
...
export default function ResourceBookingApp() {
  const [resources, setResources] = useState(RESOURCE_SEED);
  const [groups, setGroups] = useState(DEFAULT_GROUPS);
  const [bookings, setBookings] = useState([]);
  const [periodsByGroup, setPeriodsByGroup] = useState(() => buildDefaultPeriodsByGroup(DEFAULT_GROUPS));
  ...
  const [editingGroupId, setEditingGroupId] = useState("g1");
  ...
  const [newResourceGroupId, setNewResourceGroupId] = useState("g1");
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        setResources(data.resources.length ? data.resources : RESOURCE_SEED);
        setGroups(data.groups.length ? data.groups : DEFAULT_GROUPS);
        setBookings(data.bookings.length ? data.bookings : SAMPLE_BOOKINGS());
        setPeriodsByGroup(Object.keys(data.periodsByGroup).length ? data.periodsByGroup : buildDefaultPeriodsByGroup(data.groups.length ? data.groups : DEFAULT_GROUPS));
        setUsers(data.users.length ? data.users : USER_SEED);
        setNotifications(data.notifications);
      } catch (e) {
        console.error("Failed to load from Supabase — check your .env credentials and that supabase/schema.sql has been run.", e);
        if (!cancelled) setBookings(SAMPLE_BOOKINGS());
      } finally {
  ...
  function periodsFor(groupId) { return periodsByGroup[groupId] || []; }
  ...
  function addGroup() {
    ...
    if (groups.some((g) => g.name.toLowerCase() === name.toLowerCase())) { setResourcePanelError("A group with that name already exists."); return; }
    const ng = { id: uid(), name };
    const newPeriods = clonePeriods(DEFAULT_PERIODS);
    setGroups((prev) => [...prev, ng]);
    setPeriodsByGroup((prev) => ({ ...prev, [ng.id]: newPeriods }));
    runPersist(async () => {
      await api.upsertGroup(ng);
      await api.replaceGroupPeriods(ng.id, newPeriods);
    });
    setNewGroupName("");
    setResourcePanelError("");
  }
